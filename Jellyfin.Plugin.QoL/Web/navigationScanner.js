  ...
  const productionApi = (function () {
    ...
    class JellyfinAirScanner {
      ...
      genericCandidateSnapshot(element) {
        ...
        let style = null;
        try { style = getComputedStyle(element); } catch (_) {}
        if (style?.cursor === 'pointer') evidence.push('cursor-pointer');
        ...
        return {
          ...
          visibility,
          evidence,
          confidence: evidence.includes('native-semantic') ? 1 : evidence.some(v => v.startsWith('role:')) ? 0.97 : evidence.includes('tabindex') ? 0.9 : evidence.includes('data-action') ? 0.86 : evidence.includes('cursor-pointer') ? 0.62 : 0.45
        };
      }
